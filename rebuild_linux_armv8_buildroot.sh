...
# Editor (tools)
scons -j2 tools=yes target=release_debug debug_symbols=no platform=x11 CCFLAGS="-mtune=cortex-a72 -mcpu=cortex-a72 -mfloat-abi=hard -mlittle-endian -munaligned-access -mfpu=neon-fp-armv8" module_denoise_enabled=no module_raycast_enabled=no module_webm_enabled=no module_theora_enabled=no 2>&1 | tee ./logs/scons_x11_armv8_tools_build.txt;

# Template(s)
scons -j2 tools=no target=release_debug debug_symbols=no platform=x11 CCFLAGS="-mtune=cortex-a72 -mcpu=cortex-a72 -mfloat-abi=hard -mlittle-endian -munaligned-access -mfpu=neon-fp-armv8" module_denoise_enabled=no module_raycast_enabled=no module_webm_enabled=no module_theora_enabled=no 2>&1 | tee ./logs/scons_x11_armv8_debug_build.txt;
