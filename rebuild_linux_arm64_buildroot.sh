...
# For incremental
export SCONS_CACHE="./scons_cache/scons_cache_x11_arm64";
mkdir -p $SCONS_CACHE
mkdir -p "./logs"
...
# Editor (tools)
scons -j2 tools=yes target=release_debug platform=x11 debug_symbols=no arch=arm64 2>&1 | tee ./logs/scons_x11_arm64_tools_build.txt;

# Template(s)
scons -j2 tools=no target=release_debug platform=x11 debug_symbols=no arch=arm64 2>&1 | tee ./logs/scons_x11_arm64_debug_build.txt;
